...
func makeAddHandler(m *mux.Router, endpoints endpoint.Endpoints, options []http.ServerOption) {
	m.Methods("GET").Path("/add").Handler(handlers.CORS(handlers.AllowedMethods([]string{"POST"}), handlers.AllowedOrigins([]string{"*"}))(http.NewServer(endpoints.AddEndpoint, decodeAddRequest, encodeAddResponse, options...)))
}
...
func decodeAddRequest(_ context.Context, r *http1.Request) (interface{}, error) {
	...
	req := endpoint.AddRequest{Id: int32(id)}
	return req, err
}
